...
const DashboardLayout = () => {
  ...
  const getUserNavigation = () => {
    if (user.role === "admin") {
      ...
      return [
        ...
        { name: "Appointments", href: "/counselor/appointments", icon: CalendarIcon },
        // { name: "Video Call", href: "/counselor/video-chat", icon: VideoCameraIcon },
        { name: "Profile", href: "/counselor/profile", icon: UserIcon },
      ]
    // } else if(user.role === "user" && (user.subscription.plan === "premium" || user.subscription.plan === "standard")){
    //   return [
    //     { name: "Dashboard", href: "/user", icon: ChartBarIcon },
    //     { name: "Counselors", href: "/user/counselors", icon: UsersIcon },
    //     { name: "Appointments", href: "/user/appointments", icon: CalendarIcon },
    //     { name: "Self-Assessment", href: "/assessment", icon: ClipboardDocumentCheckIcon },
    //     // { name: "Chat", href: "/user/chat", icon: ChatBubbleLeftRightIcon },
    //     { name: "Upgrade Plan", href: "/user/upgrade", icon: CreditCardIcon },
    //     { name: "Payment History", href: "/user/payments/history", icon: CreditCardIcon },
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      {/* Mobile sidebar */}
      <div
        className={`fixed inset-0 bg-gray-600 bg-opacity-75 z-40 transition-opacity ${sidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      />

      <div
        className={`fixed inset-y-0 left-0 w-64 bg-indigo-700 overflow-y-auto transition transform z-50 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
            ...
            <div className="p-4 border-t border-indigo-800">
              <div className="flex items-center">
                <div className="flex-shrink-0">
                  <img
                    className="h-10 w-10 rounded-full bg-indigo-600 p-1"
        ...
        <div className="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-white shadow">
          ...
          <div className="flex-1 px-4 flex justify-between">
            ...
            <div className="ml-4 flex items-center md:ml-6">
              <Link
                to={user?.role === "admin" ? "/admin/profile" : `/${user?.role}/profile`}
